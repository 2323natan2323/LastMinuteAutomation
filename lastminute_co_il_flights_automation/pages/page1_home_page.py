...
class HomePage(BasePage):
    ...
    HAMBURGER_BTN = "app-umbraco-header  .hamburger-wrapper.active"
    ...
    def safe_landing(self):
        ...
        flight_tab.wait_for(state="visible", timeout=20000)
        assert flight_tab.is_visible(), "❌ The site wasn't loaded!"
        print("✅ The site was loaded successfully!")

    def handle_prompt_and_assert_login(self, password: str):
        ...
        # וידוא הצלחה
        success_element = self._page.locator(self.HAMBURGER_BTN)
        success_element.wait_for(state="visible", timeout=10000)
        assert success_element.is_visible(), "❌ Login failed – success element not visible!"
        print("🎉 Login successful!")

    def close_cookies_message(self):

        home_page_cookies_message = self._page.locator(self.COOKIES_MESSAGE_CLOSE_BTN)
        home_page_cookies_message.wait_for(state="visible", timeout=20000)
    ...
    def set_trip_direction(self):  # Set the trip direction
        ...
        trip_type_dropdown.wait_for(state="visible", timeout=20000)
        ...
        for trip_name, trip_selector in trip_type_options.items():
            ...
            self._page.locator(trip_selector).wait_for(state="visible", timeout=20000)
        ...
        round_trip_option.wait_for(state="visible", timeout= 20000)
    ...
    def set_passenger_type_and_count(self):
        ...
        passenger_dropdown.wait_for(state="visible", timeout=20000)
        ...
        for button in adjust_passenger_count:
            self._page.locator(button).wait_for(state="visible", timeout=20000)
            self.click(button)

        passenger_dropdown_confirmation_btn = self._page.locator(self.PASSENGER_DROPDOWN_CONFIRMATION_BTN)
        passenger_dropdown_confirmation_btn.wait_for(state="visible", timeout=20000)
    ...
    def set_flight_class(self):
        ...
        for flight_name, flight_selector in flight_classes.items():
            ...
            flight_class_dropdown.wait_for(state="visible", timeout=20000)
            ...
            dropdown_options.wait_for(state="visible", timeout=20000)

            print("✅ The flight class dropdown was clicked successfully")
            self._page.locator(flight_selector).wait_for(state="visible", timeout=20000)
        ...
        economy_class.wait_for(state="visible", timeout=20000)
    ...
    def choose_outbound_flight(self, outbound_city):
        ...
        choose_outbound_flight_btn.wait_for(state="visible", timeout=20000)
        ...
        actual_outbound_city.wait_for(state="visible", timeout=20000)
    ...
    def choose_inbound_flight(self, inbound_city):
        ...
        choose_inbound_flight_btn.wait_for(state="visible", timeout=20000)
    ...
    def set_flight_dates(self, outbound_flight_homepage_date, inbound_flight_homepage_date):
        ...
        for btn in months_btn:
            self._page.locator(btn).wait_for(state="visible", timeout=20000)
            self.click(btn)
        ...
        outbound_flight_homepage = self._page.locator('.container__months .month-item:nth-child(2) .container__days a.day-item').filter(has_text=f"{outbound_flight_homepage_date}")
        outbound_flight_homepage.wait_for(state="visible", timeout=10000)
        assert outbound_flight_homepage.is_visible(), "❌ The flight is not visible!"
        print("✅ The flight was chosen successfully!")
        self.click(outbound_flight_homepage)

        inbound_flight_homepage = self._page.locator('.container__months .month-item:nth-child(2) .container__days a.day-item').filter(has_text=f"{inbound_flight_homepage_date}")
        inbound_flight_homepage.wait_for(state="visible", timeout=10000)
        assert inbound_flight_homepage.is_visible(), "❌ The flight is not visible!"
        print("✅ The flight was chosen successfully!")
        self.click(inbound_flight_homepage)
